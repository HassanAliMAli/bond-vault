    ...
    <PageTransition className="space-y-6">
      <div className="text-center mb-2">
        ...
        <p className="text-sm text-muted max-w-md mx-auto">
          {checkState === "idle" &&
            "Match your entire portfolio against historical draw results in seconds."}
          {checkState === "slot-machine" && "Scanning every bond against draw databases..."}
          {checkState === "checking" && "Processing results..."}
          {checkState === "results" &&
            `${(results?.totalChecked ?? 0).toLocaleString()} bonds checked against historical draws`}
        </p>
      </div>
        ...
        {checkState === "idle" && (
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 text-sm text-muted">
              ...
              <DenominationBadge denomination="100" />
              <DenominationBadge denomination="200" />
              <DenominationBadge denomination="750" />
              <DenominationBadge denomination="1500" />
              <DenominationBadge denomination="7500" />
              <DenominationBadge denomination="25000" />
              <DenominationBadge denomination="40000" />
            </div>
          </motion.div>
        )}
          ...
          >
            {showSlot && (
              <SlotMachine
                ...
                totalBonds={MOCK_BONDS}
              />
            )}
            {!showSlot && (
              <div className="flex items-center justify-center py-20">
                <CheckButton loading progress={0.85} totalBonds={MOCK_BONDS} />
              </div>
            )}
          </motion.div>
        )}
