...
export async function handleSubscriptionExpiration(env: Env): Promise<number> {
  ...
  const expired = await db.select().from(subscriptions).where(and(eq(subscriptions.status, "active"), lt(subscriptions.expiresAt, now))).all();

  let count = 0;
  for (const sub of expired) {
    const graceEndsAt = new Date(new Date(sub.expiresAt).getTime() + 7 * 24 * 60 * 60 * 1000).toISOString();
    if (now < graceEndsAt) {
      await db.update(subscriptions).set({ status: "grace_period", graceEndsAt, updatedAt: now }).where(eq(subscriptions.id, sub.id));
    } else {
