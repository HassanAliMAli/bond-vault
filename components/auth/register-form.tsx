...
function PasswordStrength({ password }: { password: string }) {
  ...
  const strengthColor = ["", "bg-orange", "bg-amber", "bg-green", "bg-green"];
...
export function RegisterForm() {
  ...
  return (
    <motion.form onSubmit={handleSubmit} className="space-y-5" variants={formStagger} initial="hidden" animate="visible">
      {error && (
        <motion.div className="bg-orange/10 border-2 border-orange text-black px-4 py-3 rounded-[var(--radius-sm)] text-sm font-medium"
          initial={{ opacity: 0, height: 0, y: -8 }} animate={{ opacity: 1, height: "auto", y: 0 }} transition={{ type: "spring" as const, stiffness: 500, damping: 30 }}>
          {error}
        </motion.div>
      )}
      ...
      <motion.div variants={formItem}>
        <Input label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters" autoComplete="new-password" />
        <PasswordStrength password={password} />
      </motion.div>
      <motion.div variants={formItem}>
        <Input label="Confirm password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter your password" autoComplete="new-password"
          error={passwordMismatch ? "Passwords do not match" : undefined} />
        {confirmPassword && !passwordMismatch && (
          <motion.p className="mt-1.5 text-xs text-green flex items-center gap-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <Check className="h-3 w-3" /> Passwords match
          </motion.p>
        )}
      </motion.div>
      <motion.div variants={formItem} className="pt-2">
        <Button type="submit" variant="primary" size="xl" className="w-full" loading={loading}>Create Account</Button>
      </motion.div>
      <motion.div variants={formItem} className="text-center">
        <p className="text-sm text-muted">Already have an account?{" "}
          <Link href="/login" className="text-cyan hover:text-black font-medium underline-offset-2 hover:underline transition-colors">Sign in</Link>
        </p>
      </motion.div>
    </motion.form>
  );
