...
export function SlotMachine({ isRunning, onComplete, totalBonds }: SlotMachineProps) {
  ...
  const randomDenomination = () => ["100", "200", "750", "1500", "7500", "25000", "40000"][Math.floor(Math.random() * 7)];
    ...
    <div className="flex flex-col items-center gap-6 py-12">
      ...
      >
        ...
        <div className="relative z-10 space-y-2 text-center">
          ...
          <motion.p key={currentBond} className="text-4xl font-bold text-white tracking-[0.3em]"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.08 }}>
            {currentBond}
          </motion.p>
          <motion.p key={currentDenom} className="text-sm text-cyan/70 font-medium"
      ...
      <div className="text-center">
        ...
        <div className="flex gap-1 justify-center mt-3">
          {[1, 2, 3].map((dot) => (
            <motion.div key={dot} className="w-2 h-2 rounded-full bg-cyan"
              animate={phase !== "complete" ? { opacity: [0.3, 1, 0.3] } : { opacity: 1 }}
              transition={phase !== "complete" ? { repeat: Infinity, duration: 0.8, delay: dot * 0.15 } : {}}
            />
          ))}
        </div>
      </div>
