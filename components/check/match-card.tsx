...
const prizeIcons: Record<string, React.ReactNode> = {
  "1st Prize": <Trophy className="h-5 w-5 text-gold-500" />,
...
const prizeColors: Record<string, string> = {
  "1st Prize": "border-l-gold-500 bg-gradient-to-r from-gold-50/50 to-transparent",
...
export function MatchCard({
  ...
  return (
    ...
    >
      <Card
        variant="outlined"
        className={`border-l-4 ${prizeColors[prizeType] || "border-l-gold-500"} p-4`}
      >
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-white border border-[var(--border)] flex items-center justify-center shrink-0">
            {prizeIcons[prizeType] || <Trophy className="h-5 w-5 text-gold-500" />}
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="font-mono text-base font-semibold text-warm-900">
                #{bondNumber}
              </span>
              <DenominationBadge denomination={denomination} />
            </div>
            <div className="flex items-center gap-2 text-sm">
              <span className="font-medium text-warm-700">{prizeType}</span>
