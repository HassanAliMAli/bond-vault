...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, interactive = false, variant = "elevated", children, ...props }, ref) => {
    const baseClasses =
      "rounded-[var(--radius-md)] p-5 transition-colors";
    ...
    const variantClasses = {
      ...
      filled:
        "bg-warm-50",
      glass:
        "bg-white/60 backdrop-blur-xl border border-white/20 shadow-elevation-1",
    };
...
const CardTitle = ({
  ...
  <h3
    className={cn("font-display text-lg font-semibold leading-tight tracking-tight text-warm-900", className)}
    {...props}
  />
);
