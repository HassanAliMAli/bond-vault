...
interface SheetProps {
  ...
  title?: string; description?: string; side?: "bottom" | "right"; className?: string;
}
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div
            className={cn("fixed z-50 bg-dark-800 border border-dark-600 shadow-elevation-4 p-6 overflow-y-auto", sheetClass[side], className)}
            variants={variants[side]} initial="hidden" animate="visible" exit="exit"
            drag={side === "bottom" ? "y" : undefined} dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.1}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="ghost" size="icon" onClick={onClose} className="shrink-0"><X className="h-5 w-5" /></Button>
            </div>
