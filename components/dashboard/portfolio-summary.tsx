...
const statCards = [
  { key: "total", label: "Total Bonds", icon: Vault, accent: "text-cyan", bg: "bg-cyan/5" },
  { key: "checked", label: "Last Checked", icon: ScrollText, accent: "text-green", bg: "bg-green/5" },
  { key: "matches", label: "Winners Found", icon: Trophy, accent: "text-amber", bg: "bg-amber/5" },
];
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-3xl font-bold text-black tracking-tight">
                      {stat.key === "checked" ? "Jun 15" : <FormattedCounter to={value} duration={1.2} />}
                    </p>
                    <p className="text-xs text-muted">{stat.key === "total" ? `${value} bonds in your vault` : stat.key === "matches" ? `${value > 0 ? "🎉 " : ""}Across all draws` : "Last draw checked"}</p>
                  </div>
