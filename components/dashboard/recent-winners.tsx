  ...
  return (
    <Card variant="elevated">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Link href="/check"><Button variant="ghost-cyan" size="sm" className="gap-1">Check All <ArrowRight className="h-3.5 w-3.5" /></Button></Link>
      </CardHeader>
      <CardContent>
        {winners.length > 0 ? (
          <div className="space-y-3">
            {winners.map((w, i) => (
              <motion.div key={w.id} className="flex items-center gap-3 p-3 rounded-[var(--radius-sm)] bg-green/[0.05] border-2 border-green/20"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm font-semibold text-black">#{w.bondNumber}</span>
                    <DenominationBadge denomination={w.denomination} />
                  </div>
                  <p className="text-xs text-muted">{w.prizeType} — {w.prizeAmount} · {w.drawDate}</p>
                </div>
              </motion.div>
            ))}
          </div>
