  ...
  return (
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="rounded-[var(--radius-md)] bg-white border border-black/5 p-5 space-y-3">
            <Skeleton variant="text" className="w-20" /><Skeleton variant="text" className="h-10 w-24" /><Skeleton variant="text" className="w-32" />
          </div>
        ))}
      </div>
      <div className="rounded-[var(--radius-md)] bg-white border border-black/5 p-5 space-y-4">
        <Skeleton variant="text" className="w-40" /><Skeleton variant="rectangular" className="h-3 w-full" />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-[var(--radius-md)] bg-black/[0.02] p-4 space-y-2">
