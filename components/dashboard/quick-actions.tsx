...
const actions = [
  ...
  { href: "/bonds", label: "Manage Bonds", description: "View and organize", icon: ScrollText, accent: "bg-black/5 text-black" },
  { href: "/settings", label: "Settings", description: "Account preferences", icon: Settings, accent: "bg-black/5 text-black" },
];
          ...
          {actions.map((action, i) => (
            <motion.div key={action.href} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <Link href={action.href}
                className={`group flex flex-col items-center gap-2.5 p-4 rounded-[var(--radius-md)] border-2 border-transparent hover:border-black/10 bg-white hover:shadow-elevation-2 transition-all text-center cursor-pointer ${action.primary ? "ring-2 ring-cyan/30" : ""}`}>
