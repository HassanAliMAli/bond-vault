...
const barColors: Record<string, string> = {
  "100": "bg-black/20", "200": "bg-amber", "750": "bg-orange",
  "1500": "bg-cyan", "7500": "bg-green", "25000": "bg-amber", "40000": "bg-cyan",
};
const bgColors: Record<string, string> = {
  "100": "bg-black/[0.02]", "200": "bg-amber/[0.05]", "750": "bg-orange/[0.05]",
...
export function DenominationBreakdown({ data, total }: DenominationBreakdownProps) {
  return (
    <Card variant="elevated">
      ...
      <CardContent>
        <div className="flex h-3 rounded-full overflow-hidden mb-6 border border-black/5">
          {data.map((d) => {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {data.map((d, i) => (
            <motion.div key={d.denomination}
              className={`rounded-[var(--radius-md)] p-4 ${bgColors[d.denomination]} border-2 border-transparent hover:border-black/10 transition-all cursor-pointer group`}
